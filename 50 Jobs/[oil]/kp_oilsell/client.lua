...
function OpenOil()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Oil_shop', {
		title    = 'Oil Pack ',
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('kp_oil:sell', data.current.name, data.current.value)
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if GetDistanceBetweenCoords(coords, config.zones.dealer.coords, true) < 2 then
				...
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to ~g~Sell ~w~Oil")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 10 then
				DrawMarker(2, px, py, pz, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 0,0,0, 165, 1,0, 0,1)	
		end
		...
		if GetDistanceBetweenCoords(coords, config.zones.dealer.coords, true) < 10 then
				DrawMarker(29, p2x, p2y, p2z, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 0,0,0, 165, 1,0, 0,1)	
		end
