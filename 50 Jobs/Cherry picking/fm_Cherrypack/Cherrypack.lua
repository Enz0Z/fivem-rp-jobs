	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to ~g~Process ~y~Cherrypack")
			end
...
function Process()
	...
	TriggerServerEvent('fm_Cherrypack:process')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), config.zones.process.coords, false) > 4 then
			TriggerServerEvent('fm_Cherrypack:cancelProcessing')
			break
		end
	end
