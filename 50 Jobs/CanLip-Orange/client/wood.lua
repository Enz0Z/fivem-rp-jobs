	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('CanLip-Orange:canPickUp', function(canPickUp)
					...
					--	Citizen.Wait(2000)
						...
						TriggerServerEvent('CanLip-Orange:pickedUpwoods')
					else
...
RegisterNetEvent('CanLip-Orange:CheackingOK')
AddEventHandler('CanLip-Orange:CheackingOK', function()
	itemFound = true
end)


RegisterNetEvent('CanLip-Orange:CheackingSellOK')
AddEventHandler('CanLip-Orange:CheackingSellOK', function(itemValue)
	itemFoundSell = true
	itemValue = 1
end)

RegisterNetEvent('CanLip-Orange:CheackingNO')
AddEventHandler('CanLip-Orange:CheackingNo', function()
	itemFound = false
end)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodCut.coords.x, Config.CircleZones.woodCut.coords.y, Config.CircleZones.woodCut.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            TriggerServerEvent('CanLip-Orange:CheackingPack')
            ...
            if itemFound == true  then
            	...
            	procent(50)
             	TriggerServerEvent('CanLip-Orange:pack')
            	ClearPedTasks(playerPed)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodSell.coords.x, Config.CircleZones.woodSell.coords.y, Config.CircleZones.woodSell.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            if IsControlJustReleased(1,  38) and StatusReady == true  then
            itemFoundSell = false
            TriggerServerEvent('CanLip-Orange:SellCheak')
             FreezeEntityPosition(playerPed, true)
            Citizen.Wait(500)
             FreezeEntityPosition(playerPed, false)
            if itemFoundSell == true  then
            	...
            	procent(150)
             	TriggerServerEvent('CanLip-Orange:Sellpack')
            	ClearPedTasks(playerPed)
