	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to ~g~Process ~y~fertilizer")
			end
...
function Process()
	...
	TriggerServerEvent('fm_fertilizer:process')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), config.zones.process.coords, false) > 4 then
			TriggerServerEvent('fm_fertilizer:cancelProcessing')
			break
		end
	end
