...
RegisterServerEvent('meeta_cook_harvest:pickedUp')
AddEventHandler('meeta_cook_harvest:pickedUp', function()
	...
	if xItem.limit ~= -1 and xItem.count >= xItem.limit then
		TriggerClientEvent("pNotify:SendNotification", source, {
			text = '<span class="red-text">'..Config.ItemNameText..'คุณเต็ม</span> ',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
...
ESX.RegisterServerCallback('meeta_cook_harvest:checkItem', function(source, cb, item, item2)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('meeta_cook_harvest:canPickUp', function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xItem = xPlayer.getInventoryItem(item)
