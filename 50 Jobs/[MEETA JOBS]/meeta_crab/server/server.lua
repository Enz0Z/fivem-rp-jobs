...
RegisterServerEvent('meeta_crab:pickedUp')
AddEventHandler('meeta_crab:pickedUp', function()
	...
	if xItem.limit ~= -1 and xItem.count >= xItem.limit then
		TriggerClientEvent("pNotify:SendNotification", source, {
			text = '<span class="red-text">ปู้ม้าคุณเต็ม</span> ',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
...
ESX.RegisterServerCallback('meeta_crab:checkItem', function(source, cb, item, item2)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('meeta_crab:canPickUp', function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xItem = xPlayer.getInventoryItem(item)
