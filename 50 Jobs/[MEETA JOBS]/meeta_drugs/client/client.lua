...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedField.Zone2.coords, true) < 50 then
			SpawnWeedPlants2()
			Citizen.Wait(500)
		else
			Citizen.Wait(500)
	...
	while true do
		...
		if nearbyObject1 and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp1 then
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to harvest the ~g~Cannabis~s~ plant.")
			end

			if IsControlJustReleased(0, Config.Key['E']) and not isPickingUp1 then
				isPickingUp1 = true

				ESX.TriggerServerCallback('meeta_drugs:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('meeta_drugs:pickedUp')
					else
...
function SpawnWeedPlants2()
	while spawnedWeeds2 < 25 do
		Citizen.Wait(0)
...
function ValidateWeedCoord2(plantCoord)
	if spawnedWeeds2 > 0 then
		local validate = true
		...
		if GetDistanceBetweenCoords(plantCoord, Config.CircleZones.WeedField.Zone2.coords, false) > 50 then
			validate = false
		end
...
function GenerateWeedCoords2()
	while true do
		...
		weedCoordX = Config.CircleZones.WeedField.Zone2.coords.x + modX
		weedCoordY = Config.CircleZones.WeedField.Zone2.coords.y + modY

		local coordZ = GetCoordZ(weedCoordX, weedCoordY)
		local coord = vector3(weedCoordX, weedCoordY, coordZ)

		if ValidateWeedCoord2(coord) then
			return coord
		end
	end
end
