...
RegisterServerEvent('meeta_drugs:pickedUp')
AddEventHandler('meeta_drugs:pickedUp', function()
	...
	if xItem.limit ~= -1 and (xItem.count + xItemCount) > xItem.limit then
		TriggerClientEvent("pNotify:SendNotification", source, {
			text = '<strong class="red-text">'..xItem.label..' ของคุณเต็ม</strong> ',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
...
ESX.RegisterServerCallback('meeta_drugs:checkPolice', function(source, cb)
...
ESX.RegisterServerCallback('meeta_drugs:canPickUp', function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
