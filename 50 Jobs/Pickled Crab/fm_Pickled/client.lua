...
function OpenOpium()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Pickled_shop', {
		title    = 'Pickled',
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('fm_Pickled:sell', data.current.name, data.current.value)
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if GetDistanceBetweenCoords(coords, config.zones.dealer.coords, true) < 2 then
				...
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to ~g~Sell ~y~Pickled")
