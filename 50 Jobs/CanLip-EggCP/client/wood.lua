	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('CanLip-EggCP:canPickUp', function(canPickUp)
					...
					--	Citizen.Wait(2000)
						...
						TriggerServerEvent('CanLip-EggCP:pickedUpwoods')
					else
...
RegisterNetEvent('CanLip-EggCP:CheackingOK')
AddEventHandler('CanLip-EggCP:CheackingOK', function()
	itemFound = true
end)


RegisterNetEvent('CanLip-EggCP:CheackingSellOK')
AddEventHandler('CanLip-EggCP:CheackingSellOK', function(itemValue)
	itemFoundSell = true
	itemValue = 1
end)

RegisterNetEvent('CanLip-EggCP:CheackingNO')
AddEventHandler('CanLip-EggCP:CheackingNo', function()
	itemFound = false
end)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodCut.coords.x, Config.CircleZones.woodCut.coords.y, Config.CircleZones.woodCut.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            TriggerServerEvent('CanLip-EggCP:CheackingPack')
            ...
            if itemFound == true  then
            	...
            	procent(50)
             	TriggerServerEvent('CanLip-EggCP:pack')
            	ClearPedTasks(playerPed)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodSell.coords.x, Config.CircleZones.woodSell.coords.y, Config.CircleZones.woodSell.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            if IsControlJustReleased(1,  38) and StatusReady == true  then
            itemFoundSell = false
            TriggerServerEvent('CanLip-EggCP:SellCheak')
             FreezeEntityPosition(playerPed, true)
            Citizen.Wait(500)
             FreezeEntityPosition(playerPed, false)
            if itemFoundSell == true  then
            	...
            	procent(150)
             	TriggerServerEvent('CanLip-EggCP:Sellpack')
            	ClearPedTasks(playerPed)
...
function Spawnwoods()
	while spawnedshellfish < 70 do
		...
		ESX.Game.SpawnLocalObject('prop_alien_egg_01', woodCoords, function(obj)
