	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('CanLip-Apple:canPickUp', function(canPickUp)
					...
					--	Citizen.Wait(2000)
						...
						TriggerServerEvent('CanLip-Apple:pickedUpwoods')
					else
...
RegisterNetEvent('CanLip-Apple:CheackingOK')
AddEventHandler('CanLip-Apple:CheackingOK', function()
	itemFound = true
end)


RegisterNetEvent('CanLip-Apple:CheackingSellOK')
AddEventHandler('CanLip-Apple:CheackingSellOK', function(itemValue)
	itemFoundSell = true
	itemValue = 1
end)

RegisterNetEvent('CanLip-Apple:CheackingNO')
AddEventHandler('CanLip-Apple:CheackingNo', function()
	itemFound = false
end)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodCut.coords.x, Config.CircleZones.woodCut.coords.y, Config.CircleZones.woodCut.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            TriggerServerEvent('CanLip-Apple:CheackingPack')
            ...
            if itemFound == true  then
            	...
            	procent(50)
             	TriggerServerEvent('CanLip-Apple:pack')
            	ClearPedTasks(playerPed)
  ...
  if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, Config.CircleZones.woodSell.coords.x, Config.CircleZones.woodSell.coords.y, Config.CircleZones.woodSell.coords.z, true) < 4) then
		...
		local playerPed = PlayerPedId()
          ...
          if distance < 2 then
            ...
            if IsControlJustReleased(1,  38) and StatusReady == true  then
            itemFoundSell = false
            TriggerServerEvent('CanLip-Apple:SellCheak')
             FreezeEntityPosition(playerPed, true)
            Citizen.Wait(500)
             FreezeEntityPosition(playerPed, false)
            if itemFoundSell == true  then
            	...
            	procent(150)
             	TriggerServerEvent('CanLip-Apple:Sellpack')
            	ClearPedTasks(playerPed)
...
function Spawnwoods()
	while spawnedshellfish < 70 do
		...
		ESX.Game.SpawnLocalObject('prop_tree_olive_cr2', woodCoords, function(obj)
